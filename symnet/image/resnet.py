# Code adapted from https://keras.io/examples/cifar10_resnet/
...
class ResNet(AbstractImageClassificationModel):
    ...
    def __init__(self, path: str, n: int = 18, version: int = 2, *args, **kwargs):
        """
        Initializes the ResNet class
        :param path: The path to the CSV file
        :param n: Controls the depth of the ResNet
        :param version: Controls the ResNet version
        ...
        if version == 1:
            ...
            raise ValueError("version must be 1 or 2 only.")

        super(ResNet, self).__init__(path, *args, **kwargs)
    ...
    def resnet_layer(inputs,
        ...
        if conv_first:
            ...
            if activation is not None:
                x = CustomActivation(activation)(x)
        else:
            if batch_normalization:
                x = BatchNormalization()(x)
            if activation is not None:
    ...
    def resnet_v1(self, input_shape, depth):
        ...
        x = self.resnet_layer(inputs=inputs, activation=self.activation)
